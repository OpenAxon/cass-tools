...
{
    public static Logger logger = LoggerFactory.getLogger(MyBulkLoader.class);
    ...
    Options are:
     ...
     -d,--nodes <initial hosts>                          try to connect to
                                                         these hosts (comma separated) initially for ring information
     -h,--help                                           display this help
                                                         message
     -i,--ignore <NODES>                                 don't stream to this
                                                         (comma separated) list of nodes
     -ks,--keystore <KEYSTORE>                           SSL: full path to
                                                         keystore
     -kspw,--keystore-password <KEYSTORE-PASSWORD>       SSL: password of the
                                                         keystore
        --no-progress                                    don't display
                                                         progress
     -p,--port <rpc port>                                port used for rpc
                                                         (default 9160)
     ...
     -ts,--truststore <TRUSTSTORE>                       SSL: full path to
                                                         truststore
     -tspw,--truststore-password <TRUSTSTORE-PASSWORD>   SSL: password of the
                                                         truststore
    ...
    public static void doBulkLoad(String args[]) throws BackupRestoreException
    {
        ...
        DatabaseDescriptor.setCounterWriteRpcTimeout(30000l);
        DatabaseDescriptor.setRangeRpcTimeout(60000l);
        DatabaseDescriptor.setWriteRpcTimeout(60000l);
        ...
        catch (Exception e)
        {
            ...
            throw new BackupRestoreException(scala.Option.apply(new Object()), scala.Option.apply(e.getMessage()));
        }
        ...
        catch (Exception e)
        {
            ...
            logger.error(loader.getFailedHosts().toString());
            //e.printStackTrace(System.err);
            throw new BackupRestoreException(scala.Option.apply(new Object()), scala.Option.apply(e.getMessage()));
        }
    }
    ...
    static class LoaderOptions
    {
        ...
        public static LoaderOptions parseArgs(String cmdArgs[]) throws BackupRestoreException
        {
            ...
            try
            {
                ...
                if (cmd.hasOption(HELP_OPTION))
                {
                    printUsage(options);
                    //System.exit(0);
                    throw new BackupRestoreException(scala.Option.apply(new Object()), scala.Option.apply("Too many arguments"));
                }

                String[] args = cmd.getArgs();
                if (args.length == 0)
                {
                    logger.error("Missing sstable directory argument");
                    printUsage(options);
                    //System.exit(0);
                    throw new BackupRestoreException(scala.Option.apply(new Object()), scala.Option.apply("Missing sstable directory argument"));
                }
                ...
                {
                    logger.error("Initial hosts must be specified (-d)");
                    printUsage(options);
                    //System.exit(1);
                    throw new BackupRestoreException(scala.Option.apply(new Object()), scala.Option.apply("Initial hosts must be specified (-d)"));
        ...
        private static void errorMsg(String msg, CmdLineOptions options) throws BackupRestoreException
        {
            logger.info(msg);
            printUsage(options);
            throw new BackupRestoreException(scala.Option.apply(new Object()), scala.Option.apply(msg));
        }
